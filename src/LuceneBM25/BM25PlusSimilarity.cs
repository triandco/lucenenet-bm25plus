...
namespace LuceneSimilarities
{
    ...
    /// @lucene.experimental
    /// </summary>
    public class BM25PlusSimilarity : Similarity
    {
        ...
        /// </summary>
        public virtual float AvgFieldLength(CollectionStatistics collectionStats)
        {
        ...
        /// </summary>
        public virtual byte EncodeNormValue(float boost, int fieldLength) 
        {
        ...
        /// </summary>
        public virtual float DecodeNormValue(byte b)
        {
        ...
        public static readonly float[] NORM_TABLE = LoadNormTable();
        ...
        public float[] getCache(float avgdl){
        ...
        public override sealed SimWeight ComputeWeight(float queryBoost, CollectionStatistics collectionStats, params TermStatistics[] termStats)
        {
            ...
            for (int i = 0; i < cache.Length; i++)
            {
                var docLength = DecodeNormValue((byte)i);
                cache[i] = k1 * (1f - b + b * docLength / avgdl);
            }
        ...
        public class BM25DocScorer : SimScorer
        {
            private readonly BM25PlusSimilarity outerInstance;

            private readonly BM25Stats stats;
            private readonly NumericDocValues norms;
            private readonly float[] cache;

            internal BM25DocScorer(BM25PlusSimilarity outerInstance, BM25Stats stats, NumericDocValues norms)
            {
            ...
            public override float Score(int doc, float freq)
            {
                ...
                return stats.Weight * (outerInstance.delta + (freq * (outerInstance.k1 + 1f)) / norm + freq);
            }
        ...
        public class BM25Stats : SimWeight
        {
            /// <summary>
            /// BM25's idf </summary>
        ...
        private Explanation ExplainScore(int doc, Explanation freq, BM25Stats stats, NumericDocValues norms)
        {
            ...
            if (norms is null)
            {
                ...
                tfNormExpl.Value = 1 * (delta + (freq.Value * (k1 + 1f)) / k1 + freq.Value);
            }
            else
            {
                ...
                tfNormExpl.Value = k1 * (1f - b + b * doclen / stats.Avgdl);
            }
